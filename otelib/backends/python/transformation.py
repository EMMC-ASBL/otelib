...
from oteapi.models import AttrDict, TransformationConfig
...
class Transformation(BasePythonStrategy):
    ...
    def create(self, **kwargs) -> None:
        ...
        resource_id = "transformation-" + str(uuid4())
        self.id = resource_id
    ...
    def fetch(self, session_id: str) -> bytes:
        ...
        config = TransformationConfig(**json.loads(self.cache[resource_id]))
        session_data = None if not session_id else self.cache[session_id]
        session_update = create_strategy("transformation", config)
    ...
    def initialize(self, session_id: str) -> bytes:
        ...
        strategy = create_strategy("transformation", config)
